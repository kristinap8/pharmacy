...
const store=new Vuex.Store({
    ...
    mutations:{
        addMedicine(state, data) {
         ...
         addToCart(state,medicine){
            let isMedicineExists=state.cart.find(m => m.id == medicine.id);

            if (isMedicineExists) {
                isMedicineExists.quantity ++;
                isMedicineExists.totalPrice = (isMedicineExists.quantity * isMedicineExists.price).toFixed(2);
            } else {
                state.cart.push(medicine);
        
                Vue.set(medicine, 'quantity', 1);
                Vue.set(medicine, 'totalPrice', medicine.price);
            }
             state.cartItemsCount++
         },
         deleteItemFromCart(state,medicine){
            let index = state.cart.indexOf(medicine);
            if (index > -1) {
    ...
    actions:{
        addMedicine({ commit }, medicine) {
            ...
            firebase.database().ref('medicines')
          },
          ...
          deleteItemFromCart({commit},medicine){
              commit("deleteItemFromCart",medicine)
          }
